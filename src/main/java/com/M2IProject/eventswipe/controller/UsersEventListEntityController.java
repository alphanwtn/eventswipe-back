...
public class UsersEventListEntityController {
	...
	// creating a get mapping that show all events for a specific user's userEventList
	@GetMapping("/{userid}")
	...
	private @ResponseBody List<EventEntity> getAllEventListByStatus(@PathVariable("userid") int userid, @PathVariable("STATUS") String status) {
	...
	private @ResponseBody void deleteUserEvent(@PathVariable("userid") int userId, @PathVariable("eventid") String eventId ) {
